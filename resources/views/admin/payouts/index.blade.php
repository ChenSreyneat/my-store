...
@section('title', 'Payout History - Admin')
    ...
    <div style="margin-bottom: 4rem; display: flex; justify-content: space-between; align-items: center;" class="header-stack">
        <div>
            <h1 style="font-size: clamp(2rem, 4vw, 2.8rem); font-weight: 900; font-family: 'Outfit'; color: var(--text); letter-spacing: -1.5px; margin-bottom: 0.5rem; line-height: 1.2;">
                Payout <span class="text-gradient">History</span>
            </h1>
            <p style="color: var(--text-dim); font-size: 1rem; font-weight: 500;">Complete historical log of all vendor settlements.</p>
        </div>
    </div>
        ...
        <div class="table-container">
            ...
            <thead>
                <tr style="border-bottom: 1px solid rgba(15, 23, 42, 0.05);">
                    ...
                    <th style="padding: 1.25rem 1.5rem; opacity: 0.5; font-size: 0.75rem; text-transform: uppercase; font-weight: 800; letter-spacing: 1px;">Store / Vendor</th>
            ...
            <tbody>
                ...
                <tr style="border-bottom: 1px solid rgba(15, 23, 42, 0.05); transition: background 0.3s;" onmouseover="this.style.background='rgba(15, 23, 42, 0.01)'" onmouseout="this.style.background='transparent'">
                    ...
                    <td style="padding: 1.25rem 1.5rem;">
                        <div style="font-weight: 800; font-size: 1rem; color: var(--text);">{{ $payout->store->name ?? 'Unknown Store' }}</div>
                        <div style="font-size: 0.75rem; color: var(--text-dim); font-weight: 700; margin-top: 0.2rem;">Store ID: #{{ $payout->store_id }}</div>
                    </td>
                    <td style="padding: 1.25rem 1.5rem;">
                    ...
                    <td style="padding: 1.25rem 1.5rem; text-align: right;">
                        <a href="{{ route('admin.payouts.show', $payout->id) }}" class="btn btn-outline" style="padding: 0.5rem 1rem; font-size: 0.75rem; border-radius: 8px; font-weight: 700; color: var(--primary); border-color: rgba(99,102,241,0.2); text-decoration: none; display: inline-block;">View Details</a>
                    </td>
                </tr>
                @endforeach
                @if($payouts->isEmpty())
                <tr>
                    <td colspan="5" style="padding: 6rem 1.5rem; text-align: center; color: var(--text-dim); font-weight: 700; font-size: 0.95rem;">NO PAYOUT RECORDS DETECTED</td>
                </tr>
                @endif
            </tbody>
        </table>
