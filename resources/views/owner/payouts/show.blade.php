...
@section('title', 'Payout Details - Store Owner')

@section('content')
<section style="padding-bottom: 5rem; background: var(--bg);">
    <div style="margin-bottom: 3rem;">
        <a href="{{ route('owner.payouts') }}" style="display: inline-flex; align-items: center; gap: 0.5rem; color: var(--text-dim); text-decoration: none; font-weight: 600; font-size: 0.9rem; transition: color 0.3s;" onmouseover="this.style.color='var(--primary)'" onmouseout="this.style.color='var(--text-dim)'">
    ...
    <div style="margin-bottom: 4rem; display: flex; justify-content: space-between; align-items: flex-end;" class="header-stack">
        <div>
            ...
            <p style="color: var(--text-dim); font-size: 1rem; font-weight: 500;">Details of your settlement on {{ $payout->created_at->format('M d, Y h:i A') }}</p>
        </div>
        <div>
            <div style="font-size: 0.85rem; color: var(--text-dim); font-weight: 700; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 0.3rem;">Total Received</div>
