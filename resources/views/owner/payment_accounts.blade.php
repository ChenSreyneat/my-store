...
@section('title', 'My Bakong Accounts - Owner')

@section('content')
<section>
    <div style="margin-bottom: 4rem;">
        <h1 style="font-size: 3rem; font-weight: 800; font-family: 'Outfit';">Payment <span class="text-gradient">Accounts</span></h1>
        <p style="opacity: 0.5;">Register your Bakong details to receive payments for your hardware sales.</p>
    </div>

    <!-- Registration Form -->
    <div class="glass" style="padding: 3rem; border-radius: 40px; margin-bottom: 4rem;">
        <h3 style="margin-bottom: 2rem; font-weight: 800;">Add New Account</h3>
        <form action="{{ route('owner.payment_accounts.store') }}" method="POST" style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 2rem; align-items: end;">
            @csrf
            <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                <label style="font-weight: 700; opacity: 0.7;">Bakong ID</label>
                <input type="text" name="account_id" required placeholder="yourname@bank" style="background: var(--glass-bg); border: 1px solid var(--glass-border); padding: 1rem; border-radius: 12px; color: var(--text); width: 100%;">
            </div>
            <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                <label style="font-weight: 700; opacity: 0.7;">Account Name</label>
                <input type="text" name="account_name" required placeholder="Full Name" style="background: var(--glass-bg); border: 1px solid var(--glass-border); padding: 1rem; border-radius: 12px; color: var(--text); width: 100%;">
            </div>
            <div style="display: flex; flex-direction: column; gap: 0.5rem;">
    ...
    <div class="glass" style="padding: 3rem; border-radius: 40px;">
        <h3 style="margin-bottom: 2rem; font-weight: 800;">My Registered Accounts</h3>
            ...
            <table style="width: 100%; border-collapse: collapse; text-align: left;">
                ...
                <tbody>
                    ...
                    <tr style="border-bottom: 1px solid var(--glass-border);">
                        ...
                        <td style="padding: 1.5rem;">
                            <form action="{{ route('owner.payment_accounts.destroy', $account->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                    ...
                    @if($accounts->isEmpty())
                    <tr>
                        <td colspan="4" style="padding: 4rem; text-align: center; opacity: 0.5;">You haven't registered any payment accounts yet.</td>
                    </tr>
                    @endif
                </tbody>
            </table>
